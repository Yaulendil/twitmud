...
class wPart(TreasureObject):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.HP = self.baseDurability * self.MatStat["Toughness"] # "Health" of a part determines when it breaks and what its damage adjective is
    ...
    def damageRating(self,split=True): # Amount of damage contributed by this component
        D = []
        try:
            dBase = self.MatStat # Damage bases
            dScale = self.mapDamage # Damage scale factors
            #DB(self,dBase,dScale)
            for n in dScale: # Do the following for EACH STAT and then add it to the list
                #DB(n,dScale[n],D)
                D.append(float(dScale[n] # wp.mapDamage["Density"] (0.5)
                    * dBase[n] #    MULTIPLIED by dBase["Density"] (10)
                    * self.baseDamage)) # AND by base damage of the component class
...
class Weapon_Shortsword(Weapon):
    components = {"Blade":wp_Slice_2_small,
                  "Handle":wp_Handle,
                  "Pommel":wp_Pommel,
                  "Guard":wp_Crossguard}
    Weights = ["Blade","Guard"]
...
class Weapon_Dagger(Weapon):
    components = {"Blade":wp_Slice_1_small,
                  "Handle":wp_Handle,
                  "Pommel":wp_Pommel,
                  "Guard":wp_RoundGuard}
    Weights = ["Blade","Guard"]
...
def SWORD(*args,**kwargs):
    weps = [Weapon_Longsword,Weapon_Shortsword,Weapon_Scimitar,
            Weapon_Dagger,Weapon_Rondel,
            Weapon_Greatsword]
